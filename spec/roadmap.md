...
The full goal, format, and rules of MVP 1 live in [scope.md](scope.md), [../docs/checklist-generation.md](../docs/checklist-generation.md), and [../docs/knowledge-base.md](../docs/knowledge-base.md).
...
This is a different artifact from MVP 1 reminders and requires its own format and process. See [../docs/regressions.md](../docs/regressions.md) for the breadcrumb authoring format used today.

Seed material for MVP 2 — content that already exists but is too detailed for MVP 1 — is collected in [mvp2-regression-seeds.md](mvp2-regression-seeds.md). The seeds are not read by the assistant.

## Smaller deferred items

- Render `Regression Breadcrumbs` in `kb_tool.py` fallback output. The script today does not surface `Linked Regression Issues` sections from `knowledge/regressions/` modules. The agent workflow handles this directly, so this is a small follow-up for the script.
- Allow `kb_tool.py` to skip non-module Markdown files (for example, README files) under `knowledge/`. Today the loader requires front matter on every `*.md` it finds.
